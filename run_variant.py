...
if os.path.exists(baseline_ckpt_path):
    ...
    bl_log = json.load(open(bl_log_path)) if os.path.exists(bl_log_path) else {"loss": []}
else:
    ...
    ckpt_data = {"model": model.state_dict(), "ema": ema.state_dict(),
...
for epoch in range(1, MCL_EPOCHS + 1):
    ...
    if epoch % 5 == 0 or epoch == MCL_EPOCHS:
        ...
        print(f"  [{TAG}] epoch {epoch:3d}/{MCL_EPOCHS}  loss={avg:.4f}  usage={[f'{u:.2f}' for u in uf]}{extra}")
...
centres, usage_data = expert_vs_sigma(ema_experts, K, train_loader, SIGMA_MIN, SIGMA_MAX, device, num_batches=30)
...
axes[1].set_title(f"MCL K={K} ({TAG}) Training Loss"); axes[1].grid(True, alpha=0.3)
plt.tight_layout()
...
ax.set_title(f"Expert Usage ({TAG})"); ax.legend(); ax.grid(True, alpha=0.3)
plt.tight_layout()
...
axes[0].set_ylabel("FID ↓"); axes[0].set_title(f"FID ({TAG})")
...
axes[1].set_ylabel("Precision ↑"); axes[1].set_title(f"Precision ({TAG})")
...
axes[2].set_ylabel("Recall ↑"); axes[2].set_title(f"Recall ({TAG})")
