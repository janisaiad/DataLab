...
def generate_baseline(
    ...
    return (
        samples.clamp(-1, 1) if not return_trajectory else (samples.clamp(-1, 1), traj)
    )
...
def main():
    ...
    p.add_argument(
        "--num_steps", type=int, default=200, help="Number of ODE discretisation steps"
    )
    ...
    if args.mode == "baseline":
        ...
        if args.strategy == "gated" and args.gating_ckpt:
            g_ckpt = torch.load(
                args.gating_ckpt, map_location=device, weights_only=False
            )
