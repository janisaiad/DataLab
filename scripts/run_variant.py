...
for d in [CKPT, OUT, ANALYSIS, SHARED_CKPT]:
...
if os.path.exists(baseline_ckpt_path):
    print(f"\n[{TAG}] Reusing baseline from {baseline_ckpt_path}")
    ...
    bl_log_path = f"{SHARED_CKPT}/baseline_log.json"
    bl_log = (
        json.load(open(bl_log_path)) if os.path.exists(bl_log_path) else {"loss": []}
    )
else:
    print(
        f"\n{'=' * 60}\n  [{TAG}] 1/7  Baseline training ({BASELINE_EPOCHS} epochs)\n{'=' * 60}"
    )
    t0 = time.time()
    ...
    for epoch in range(1, BASELINE_EPOCHS + 1):
        ...
        if epoch % 5 == 0 or epoch == BASELINE_EPOCHS:
            ...
            save_image_grid(samples, f"{CKPT}/baseline_samples_ep{epoch}.png")
            print(f"  [{TAG}] epoch {epoch:3d}/{BASELINE_EPOCHS}  loss={avg:.4f}")
    ...
    torch.save(ckpt_data, f"{CKPT}/baseline_final.pt")
    with open(f"{CKPT}/baseline_log.json", "w") as f:
        json.dump(bl_log, f)
    print(f"  [{TAG}] Baseline done in {(time.time() - t0) / 60:.1f} min")
...
for epoch in range(1, MCL_EPOCHS + 1):
    ...
    for images, _ in train_loader:
        ...
        if MCL_VARIANT == "hard_wta":
            ...
            for k in range(K):
                ...
                scalers_mcl[k].step(opts[k])
                scalers_mcl[k].update()
            ...
            for k in range(K):
                ...
                with autocast("cuda", enabled=USE_AMP):
                    ...
                    loss_l = (
                        RELAXED_ALPHA * per_sample[mask_lose].sum()
                        if n_lose > 0
                        else 0.0
                    )
            ...
            with torch.no_grad():
                scores = torch.stack(
                    [scoring_heads[k](xt, sigma) for k in range(K)], dim=1
                )
    ...
    if epoch % 5 == 0 or epoch == MCL_EPOCHS:
        ...
        print(
            f"  [{TAG}] epoch {epoch:3d}/{MCL_EPOCHS}  loss={avg:.4f}  usage={[f'{u:.2f}' for u in uf]}{extra}"
        )
...
with open(f"{CKPT}/mcl_K{K}_log.json", "w") as f:
    json.dump(mcl_log, f)
print(f"  [{TAG}] MCL done in {(time.time() - t0) / 60:.1f} min")
...
for epoch in range(1, GATING_EPOCHS + 1):
    ...
    if epoch % 5 == 0 or epoch == GATING_EPOCHS:
        print(f"  [{TAG}] epoch {epoch:3d}/{GATING_EPOCHS}  acc={correct / total:.3f}")

torch.save({"gating_net": gating.state_dict(), "K": K}, f"{CKPT}/gating_K{K}.pt")
gating.eval()
print(f"  [{TAG}] Gating done in {(time.time() - t0) / 60:.1f} min")
...
print(f"  [{TAG}] baseline (euler) ...", flush=True)
...
print(f"  [{TAG}] baseline (heun) ...", flush=True)
...
for strat in [
    ...
    print(f"  [{TAG}] mcl {strat} ...", flush=True)
...
for eid in range(K):
    print(f"  [{TAG}] expert {eid} grid ...", flush=True)
    s = generate_mcl(
        ema_experts,
        K,
...
for name, imgs in gen.items():
    ...
    print(f"  [{TAG}] {name:20s}  FID={fid:8.2f}  P={prec:.4f}  R={rec:.4f}")
...
print(f"  [{TAG}] expert vs digit ...", flush=True)
...
print(f"  [{TAG}] expert vs sigma ...", flush=True)
centres, usage_data = expert_vs_sigma(
    ema_experts, K, train_loader, SIGMA_MIN, SIGMA_MAX, device, num_batches=30
)
plot_expert_vs_sigma(centres, usage_data, f"{ANALYSIS}/expert_vs_sigma.png")

print(f"  [{TAG}] multi-expert grid ...", flush=True)
all_s = same_noise_multi_expert(
    ema_experts,
    K,
    device,
    SIGMA_MIN,
...
print(f"  [{TAG}] trajectory ...", flush=True)
_, traj = generate_mcl(
    ema_experts,
    K,
    strategy="single_expert",
...
print(f"  [{TAG}] strategy comparison ...", flush=True)
...
axes[1].set_title(f"MCL K={K} ({TAG}) Training Loss")
...
for k in range(K):
    ax.plot(
        range(1, len(usage_arr) + 1), usage_arr[:, k], label=f"Expert {k}", linewidth=2
    )
ax.set_xlabel("Epoch")
ax.set_ylabel("Usage fraction")
ax.set_title(f"Expert Usage ({TAG})")
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(f"{OUT}/expert_usage_training.png", dpi=150, bbox_inches="tight")
plt.close()
...
axes[1].set_title(f"Precision ({TAG})")
...
axes[2].set_title(f"Recall ({TAG})")
axes[2].grid(True, alpha=0.3, axis="y")

plt.tight_layout()
plt.savefig(f"{OUT}/metrics_comparison.png", dpi=150, bbox_inches="tight")
plt.close()
