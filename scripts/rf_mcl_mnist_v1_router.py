...
def device_of(s: str):
    return torch.device(
        "cuda"
        if s == "auto" and torch.cuda.is_available()
        else ("cpu" if s == "auto" else s)
    )
...
def mi_norm(
    assign: torch.Tensor, labels: torch.Tensor, k: int, c: int, eps: float = 1e-12
) -> float:
...
def load_mnist(
    ...
    if torchvision is None:
        raise ImportError("torchvision is required for MNIST.")
    tr = torchvision.datasets.MNIST(
        data_root, train=True, download=not no_download, transform=T.ToTensor()
    )
    te = torchvision.datasets.MNIST(
        data_root, train=False, download=not no_download, transform=T.ToTensor()
    )
    ...
    xte = (xte - mean) / std
    info = dict(
        classes=[str(i) for i in class_ids],
...
def diffuse_with_target(x0: torch.Tensor, t: float):
    ...
    xt = g * x0 + math.sqrt(max(1.0 - g * g, 1e-12)) * eps
    return xt, eps
...
class RandomFeatures(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "W", torch.randn(p, d, generator=gen) * rf_scale / math.sqrt(d)
        )
    ...
    def forward(self, x):
        ...
        if self.activation == "erf":
            ...
            h = torch.sin(z + self.b.to(x.device))
        else:
...
class RFExperts(nn.Module):
    def __init__(self, p: int, dout: int, k: int, init_std: float):
        super().__init__()
        self.A = nn.Parameter(init_std * torch.randn(k, p, dout))
...
def mcl_loss(e, beta: float, balance: float = 0.0, entropy: float = 0.0):
    k = e.shape[1]
    if beta <= 1e-12:
    ...
    if balance > 0:
        u = q.mean(0)
        loss = loss + balance * ((u - 1.0 / k) ** 2).sum()
    if entropy > 0:
        ent = -(q.clamp_min(1e-12) * q.clamp_min(1e-12).log()).sum(1).mean() / math.log(
            k
        )
...
def sched(step: int, variant: str, beta_final: float, warm: int, ramp: int):
    if variant == "uniform":
        return 0.0
    if (
        variant.startswith("fixed")
        or variant.startswith("grid")
        or variant == "hard_cold"
    ):
        return beta_final
    if step < warm:
        return 0.0
    u = 1.0 if ramp <= 0 else min(1.0, max(0.0, (step - warm) / ramp))
    u = u * u * (3 - 2 * u)
    return beta_final * u
...
def calibrate(
    ...
    return dict(
        ...
        class_basis_rank=int(b.shape[1]),
        window_class_before_trans=float(bc < bt),
        window_class_before_glass=float(bc < bg),
    )
...
def evaluate(
    ...
    k = experts.A.shape[0]
    sums = {z: 0.0 for z in ["best", "soft", "mean", "rmix", "rsoft"]}
    n = 0
    ao = []
    ar = []
    qt = []
    ...
    for i in range(0, x.shape[0], batch):
        ...
        bsz = xb.shape[0]
        n += bsz
        sums["best"] += float(e.min(1).values.sum())
        sums["soft"] += float((q * e).sum(1).sum())
        sums["mean"] += float(e.mean(1).sum())
        ao.append(e.argmin(1).cpu())
        qt.append(q.cpu())
        if router is not None:
            rq = torch.softmax(router(phi), 1)
            mix = torch.einsum("bk,bkd->bd", rq, pred)
            sums["rmix"] += float(((mix - yb) ** 2).mean(1).sum())
            sums["rsoft"] += float((rq * e).sum(1).sum())
            ar.append(rq.argmax(1).cpu())
            qr.append(rq.cpu())
    out = {
        prefix + "oracle_best_mse": sums["best"] / n,
        prefix + "soft_oracle_mse": sums["soft"] / n,
        prefix + "mean_expert_mse": sums["mean"] / n,
    }
    ycpu = labels.cpu()
    ...
    def add(name, alist, qlist):
        ...
        a = torch.cat(alist)
        q = torch.cat(qlist)
        u = torch.bincount(a, minlength=k).float()
        u = u / u.sum().clamp_min(1)
        out[prefix + name + "_usage_entropy"] = norm_entropy(u)
        out[prefix + name + "_eff_frac_min"] = float((u.min() * k).item())
        out[prefix + name + "_class_mi_norm"] = mi_norm(a, ycpu, k, c)
        out[prefix + name + "_soft_usage_entropy"] = norm_entropy(q.mean(0))

    add("oracle", ao, qt)
    if router is not None:
        out[prefix + "router_mix_mse"] = sums["rmix"] / n
        out[prefix + "router_soft_mse"] = sums["rsoft"] / n
        q_teacher = torch.cat(qt).clamp_min(1e-12)
        q_router = torch.cat(qr).clamp_min(1e-12)
        out[prefix + "router_vs_teacher_ce"] = float(
            (-(q_teacher * q_router.log()).sum(1).mean()).item()
        )
        out[prefix + "router_vs_teacher_kl"] = float(
            (q_teacher * (q_teacher.log() - q_router.log())).sum(1).mean().item()
        )
        out[prefix + "teacher_entropy_norm"] = float(
            (-(q_teacher * q_teacher.log()).sum(1).mean() / math.log(k)).item()
        )
        add("router", ar, qr)
    return out


def train_router(experts, rf, x, y, beta: float, params, device):
    router = Router(params.p, params.K, params.router_hidden, params.router_dropout).to(
        device
    )
    opt = torch.optim.AdamW(router.parameters(), lr=params.router_lr)
    experts.eval()
    for _ in range(params.router_steps):
        idx = torch.randint(0, x.shape[0], (params.router_batch_size,), device=device)
        xb = x[idx]
        yb = y[idx]
        with torch.no_grad():
            phi = rf(xb)
            e = costs(experts(phi), yb)
            _, q = mcl_loss(e, beta)
...
class Params:
    ...
    fixed_class_mult: float = 1.2
    balance_weight_theory: float = 0.0
    entropy_weight_theory: float = 0.0
    ...
    quick: bool = False
    device: str = "auto"
    outdir: str = "./mnist_v1_router"


def train_variant(
    name, beta_final: float, params: Params, rf, data, device, d: int, c: int
):
    ex = RFExperts(params.p, d, params.K, params.init_std).to(device)
    opt = torch.optim.AdamW(
        ex.parameters(), lr=params.lr, weight_decay=params.weight_decay
    )
    rows = []
    xt, yt, lt = data["x_train"], data["y_train"], data["labels_train"]
    xv, yv, lv = data["x_test"], data["y_test"], data["labels_test"]
    for step in range(params.steps + 1):
        if step % params.eval_every == 0 or step == params.steps:
            ...
            row = {"variant": name, "step": step, "beta": b}
            row.update(evaluate(ex, rf, xv, yv, lv, b, c, batch=1024, prefix="test_"))
            m = min(2048, xt.shape[0])
            row.update(
                evaluate(
                    ex, rf, xt[:m], yt[:m], lt[:m], b, c, batch=1024, prefix="train_"
                )
            )
            rows.append(row)
        ...
        phi = rf(xt[idx])
        e = costs(ex(phi), yt[idx])
        b = sched(step, name, beta_final, params.warmup_steps, params.ramp_steps)
        bw = params.balance_weight_theory if name == "theory_anneal" else 0.0
        ew = params.entropy_weight_theory if name == "theory_anneal" else 0.0
        loss, _ = mcl_loss(e, b, bw, ew)
        opt.zero_grad(set_to_none=True)
        loss.backward()
        opt.step()
    return ex, rows
...
def plot(outdir: Path, rows, router_rows):
    if plt is None or pd is None:
        return
    if rows:
        df = pd.DataFrame(rows)
        for y in [
            "test_oracle_best_mse",
            "test_oracle_class_mi_norm",
            "test_teacher_entropy_norm",
        ]:
            if y not in df:
                continue
            plt.figure()
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig(outdir / f"{y}.png", dpi=180)
            plt.close()
    if router_rows:
        df = pd.DataFrame(router_rows)
        plt.figure()
        x = np.arange(len(df))
        plt.bar(x, df["router_mix_mse"])
        plt.xticks(x, df["variant"], rotation=45, ha="right")
        plt.ylabel("router_mix_mse")
        plt.tight_layout()
        plt.savefig(outdir / "final_router_mix_mse.png", dpi=180)
        plt.close()


def run(params: Params):
    if params.quick:
        params.n_train = min(params.n_train, 2000)
        params.n_test = min(params.n_test, 800)
        params.n_calib = min(params.n_calib, 800)
        ...
        if params.d_mode == "full":
            params.d_mode = "pca"
            params.pca_dim = min(params.pca_dim, 128)
    ...
    c = len(ids)
    xtr_raw, ytr, xte_raw, yte, info = load_mnist(
        params.data_root,
        ids,
        params.n_train,
        params.n_test,
    ...
    fmap = build_feature_map(
        xtr_raw, params.d_mode, params.pca_dim, params.rp_dim, params.seed
    )
    ...
    rf = (
        RandomFeatures(d, params.p, params.activation, params.rf_scale, params.seed)
        .to(device)
        .eval()
    )
    idx = torch.randperm(xtr.shape[0], device=device)[
        : min(params.n_calib, xtr.shape[0])
    ]
    cal = calibrate(
        rf,
        xtr[idx],
        ...
        params.ridge,
        params.power_iters,
        params.beta_target_mult,
        params.beta_glass_safety,
    )
    ...
    if not (bc < min(bg, bt)):
        print(
            f"WARNING: narrow/absent window beta_class={bc:.4g}, beta_trans={bt:.4g}, beta_glass={bg:.4g}",
            flush=True,
        )
    variants = [
        ("uniform", 0.0),
        ("fixed_class", params.fixed_class_mult * bc),
        ("fixed_good", target),
        ("hard_cold", params.cold_mult_glass * bg),
        ("theory_anneal", target),
    ]
    if params.beta_grid:
        for b in sorted(
            set([0.25 * bc, 0.75 * bc, 1.25 * bc, target, 0.8 * bt, 1.2 * bt, 0.5 * bg])
        ):
            if math.isfinite(b) and b > 0:
                variants.append((f"grid_{b:.6g}", b))
    data = dict(
        x_train=xtr,
        y_train=ytr_eps,
        labels_train=ltr,
    ...
    for name, beta_final in variants:
        ...
        ex, rows = train_variant(name, beta_final, params, rf, data, device, d, c)
        all_rows.extend(rows)
        beta_eval = beta_final if name != "uniform" else max(bc, 1e-6)
        router = train_router(ex, rf, xtr, ytr_eps, beta_eval, params, device)
        final = evaluate(
            ex, rf, xte, yte_eps, lte, beta_eval, c, router=router, batch=1024
        )
        final.update(variant=name, beta_eval=beta_eval)
        router_rows.append(final)
        torch.save(
            dict(
                experts=ex.state_dict(),
                ...
                variant=name,
                beta_eval=beta_eval,
                latent_mean=lat_mean.cpu(),
                latent_std=lat_std.cpu(),
                feature_map_mode=fmap.mode,
                pca_components=None if fmap.pca is None else fmap.pca.cpu(),
                rp_matrix=None if fmap.rp is None else fmap.rp.cpu(),
                raw_mean=None if fmap.mean is None else fmap.mean.cpu(),
            ),
            out / f"checkpoint_{name}.pt",
        )
        write_csv(out / "metrics_all.csv", all_rows)
        write_csv(out / "router_final_metrics.csv", router_rows)
    ...
    for r in router_rows:
        ...
        for k in [
            ...
            "router_mix_mse",
            "router_soft_mse",
            "oracle_class_mi_norm",
            "router_class_mi_norm",
            "oracle_usage_entropy",
            "router_usage_entropy",
            "teacher_entropy_norm",
            "router_vs_teacher_ce",
            "router_vs_teacher_kl",
        ]:
            if k in r:
