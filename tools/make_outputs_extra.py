...
STRAT_LABELS = {"single_expert": "Single", "random_expert": "Random",
                "best_expert": "Best-norm", "mixture_score": "Mixture", "gated": "Gated"}
STRAT_COLORS = {"single_expert": "#888888", "random_expert": "#1f77b4",
                "best_expert": "#2ca02c", "mixture_score": "#9467bd", "gated": "#d62728"}

def plot_fid_vs_K():
    """Plot FID trends versus the number of experts."""
    Ks = [2, 3, 4, 6, 8]
    ann_dirs = ["annealed_wta_K2", "annealed_wta_K3",
                REF, "annealed_wta_K6", "annealed_wta_K8"]
    res_dirs = ["resilient_mcl_K2", "resilient_mcl_K3",
                "resilient_mcl", "resilient_mcl_K6",
                "resilient_mcl_K8"]

    fig, axes = plt.subplots(1, 2, figsize=(12, 4.2), sharey=True)
    for ax, dirs, title in [(axes[0], ann_dirs, "Annealed WTA"),
        ...
        for s in STRATS:
            ...
            ax.plot(Ks, ys, marker="o", lw=1.7, label=STRAT_LABELS[s],
...
def plot_usage_histograms():
    ...
    for ax, (name, u) in zip(axes.flat, usages.items()):
        ...
        ax.axhline(1.0 / K, ls="--", color="black", alpha=0.6,
                   label=f"$1/K = {1/K:.2f}$")
...
def plot_wasserstein_K6():
    ...
    ax.set_title("Pairwise $W_1$ between per-expert pixel distributions\n"
                 "(Annealed $K=6$, $B=512$)")
...
def plot_fid_vs_B():
    ...
    Bs = [128, 256, 512, 1024]
    dirs = ["annealed_wta_B128", "annealed_wta_B256",
            REF, "annealed_wta_B1024"]
    fig, ax = plt.subplots(figsize=(6.2, 4.2))
    for s in STRATS:
        ys = [load_metrics(d)[s]["fid"] for d in dirs]
        ax.plot(Bs, ys, marker="o", lw=1.7, label=STRAT_LABELS[s],
                color=STRAT_COLORS[s])
    base = load_metrics(REF)["baseline_heun"]["fid"]
    ax.axhline(base, ls="--", color="black", alpha=0.6, label="Baseline (Heun)")
    ax.set_xscale("log")
...
def plot_pr_scatter():
    ...
    for s in STRATS:
        ...
        ax.scatter(xs, ys, color=STRAT_COLORS[s], s=55, alpha=0.85,
                   label=STRAT_LABELS[s], edgecolors="white", linewidths=0.7)
    m = load_metrics(REF)
    ax.scatter([m["baseline_heun"]["recall"]], [m["baseline_heun"]["precision"]],
               color="black", marker="*", s=200, label="Baseline (Heun)",
               zorder=5)
...
def plot_single_to_gated_ratio():
    ...
    for patch, i in zip(ax.patches, order):
        w = patch.get_width()
        ax.text(w * 1.05, patch.get_y() + patch.get_height() / 2,
                f"{w:.1f}$\\times$", va="center", fontsize=8)
...
def plot_gated_vs_alive():
    ...
    for name, d, alive, tag in points:
        ...
        if m is None:
            continue
        fid = m["gated"]["fid"]
        ax.scatter(alive, fid, color=color_map[tag], s=80, alpha=0.85,
                   edgecolors="white", linewidths=0.7)
        ax.annotate(name, (alive, fid), xytext=(5, 4),
    ...
    legend_elems = [
        Line2D([0], [0], marker="o", color="w", markerfacecolor="#d62728",
               markersize=9, label="Effectively single-expert"),
        Line2D([0], [0], marker="o", color="w", markerfacecolor="#2ca02c",
               markersize=9, label="Genuinely multi-expert"),
    ]
